...
std::string TcpConnect::ReceiveData(size_t bufferSize) const
{
    ...
    {
        uint32_t length;
        ssize_t len_received = recv(sock_, &length, sizeof(length), MSG_WAITALL);
        if (len_received < 0)
        {
            throw std::runtime_error("_Failed to receive length");
        }
        length = BytesToInt(reinterpret_cast<char *>(&length));
        bufferSize = length;
    }

    std::string data(bufferSize, '\0');
    ssize_t bytes_received = recv(sock_, &data[0], bufferSize, MSG_WAITALL);
    if (bytes_received < 0)
    {
