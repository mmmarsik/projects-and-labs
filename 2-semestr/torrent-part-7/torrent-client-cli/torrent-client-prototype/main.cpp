...
void CheckDownloadedPiecesIntegrity(const std::filesystem::path &outputFilename, const TorrentFile &tf, PieceStorage &pieces)
{
    ...
    for (size_t pieceIndex : pieceIndices)
    {
        ...
        {
            std::cerr << "File piece with index " << pieceIndex << " started at position " << positionInFile << " with length " << pieceDataFromFile.length() << " has wrong hash " << HexEncode(realHash) << ". Expected hash is " << HexEncode(tf.pieceHashes[pieceIndex]) << std::endl;
...
bool RunDownloadMultithread(PieceStorage &pieces, const TorrentFile &torrentFile, const std::string &ourId, const TorrentTracker &tracker)
{
    ...
    for (std::thread &thread : peerThreads)
    {
        std::cerr << "JOINING ONE " << std::endl;

        thread.join();
    }

    std::cerr << "JOINED THEM ALL" << std::endl;
    std::cerr << "JOINED THEM ALL" << std::endl;
    std::cerr << "JOINED THEM ALL" << std::endl;
    std::cerr << "JOINED THEM ALL" << std::endl;
    std::cerr << "JOINED THEM ALL" << std::endl;
...
void TestTorrentFile(const fs::path &file, const fs::path outputDirectory)
{
    ...
    PiecesToDownload = PiecesToDownload*torrentFile.pieceHashes.size()/100;
...
{
    std::string path_dir_, path_file_ = "";
    size_t percents_to_dowload = 0;

    path_dir_ = argv[2];
    percents_to_dowload = std::stoi(argv[4]);
    path_file_ = argv[5];
    PiecesToDownload = percents_to_dowload;

    TestTorrentFile(path_file_, path_dir_);
    return 0;
}
