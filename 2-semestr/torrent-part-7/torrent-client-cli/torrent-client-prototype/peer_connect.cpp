...
PeerConnect::PeerConnect(const Peer &peer, const TorrentFile &tf, std::string selfPeerId, PieceStorage &pieceStorage)
    : tf_(tf), socket_(peer.ip, peer.port, 2000ms, 2000ms), selfPeerId_(selfPeerId), terminated_(false), choked_(true), pieceStorage_(pieceStorage), pendingBlock_(false), failed_(false) {}
...
{
    std::cerr << "PeerConnect:: Terminate " << std::endl;
...
void PeerConnect::MainLoop()
{
    while (!terminated_)
    {
        ...
        case MessageId::Have:
            ...
            pieceIndex = BytesToInt(message.payload);
        ...
        case MessageId::Piece:
            ...
            if (pieceInProgress_ != nullptr && pieceInProgress_->GetIndex() == pieceIndex)
            {
                ...
                std::cout << "saved block of piece in MainLoop" << std::endl;
                if (pieceInProgress_->AllBlocksRetrieved())
                {
                    std::cout << "MainLoop:: AllBlocksRetrieved" << std::endl;

                    std::cout << "Start PieceProcessed" << std::endl;
                    pieceStorage_.PieceProcessed(pieceInProgress_);
                    std::cout << "MainLoop:: PieceProcessed" << std::endl;
...
{
    std::cerr << "RequestPiece\n";
    if (pieceInProgress_ == nullptr)
    {
        pieceInProgress_ = pieceStorage_.GetNextPieceToDownload();
        while (pieceInProgress_ == nullptr || !piecesAvailability_.IsPieceAvailable(pieceInProgress_->GetIndex()))
        {
    ...
    if (!pendingBlock_)
    {
        ...
        if (block != nullptr)
        {
            ...
            std::cerr << "INDEX = " << htonl(index) << " OFFSET = " << htonl(offset) << " LENGTH  = " << htonl(length) << std::endl;
